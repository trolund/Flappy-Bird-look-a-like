...
export default class Demo extends Phaser.Scene {
    ...
    create() {
        this.pointLabel = this.add.text(30, 30, '', { font: '16px Courier', fill: '#00ff00' });

        this.bird = this.physics.add.sprite(100, 400, "bird").setScale(0.1).setGravity(0, 200);
    ...
    setGameOver() {
        this.add.text(width / 2, height / 2, 'Game Over', { font: '16px Courier', fill: '#00ff00' });
    ...
    createPips() {
        ...
        let bottomPipe = this.physics.add.sprite(width + 50, xRandom + spaceBetweenPipsY, "pipe")
                        .setScale(0.26);

        let topPipe = this.physics.add.sprite(width + 50, xRandom - spaceBetweenPipsY, "pipe")
                        .setScale(0.26)
                        .setRotation(Math.PI);

        this.physics.add.collider(this.bird, [topPipe, bottomPipe], () => this.setGameOver());
        this.aktivePipes.push({bottomPipe, topPipe, offset: Math.abs(this.xScroll) + width + 50} as Pipe);
    }
